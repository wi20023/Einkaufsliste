...
app.get('/database', (req, res) => {
    console.log("Request to load all entries from list");
...
app.delete('/database/:id', (req, res) => {
    ...
    connection.query("DELETE FROM `list` WHERE `list`.`id` = " + id + ";", function (error, results, fields) {
        if (error) {
            // we got an errror - inform the client
            console.error(error); // <- log error in server
            res.status(500).json(error); // <- send to client
...
app.post('/database', (req, res) => {
    ...
    if (typeof req.body !== "undefined" && typeof req.body.title !== "undefined" && typeof req.body.quantity !== "undefined" && typeof req.body.unit !== "undefined") {
        ...
        connection.query("INSERT INTO `list` (`id`, `title`, `quantity`, `unit`, `created_at`) VALUES (NULL, '" + title + "', '" + quantity + "', '" + unit + "', current_timestamp());", function (error, results, fields) {
            if (error) {
                // we got an errror - inform the client
                console.error(error); // <- log error in server
                res.status(500).json(error); // <- send to client
