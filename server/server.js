...
app.get('/database', (req, res) => {
    ...
    connection.query("SELECT * FROM `main list`;", function (error, results, fields) {
        if (error) {
...
app.post('/database', (req, res) => {
    ...
    if (typeof req.body !== "undefined" && typeof req.body.title !== "undefined" && typeof req.body.quantity !== "undefined" && typeof req.body.unit !== "undefined") {
        ...
        connection.query("INSERT INTO `main list` (`id`, `title`, `quantity`, `unit`, `created_at`) VALUES (NULL, '" + title + "', '" + quantity + "', '" + unit + "', current_timestamp());", function (error, results, fields) {
            if (error) {
                // we got an errror - inform the client
                console.error(error); // <- log error in server
                res.status(500).json(error); // <- send to client
